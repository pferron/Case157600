...
namespace WOW.WoodmenIllustrationService.LookupBuilders
{
    public class CommonLookups
    {
        ...
        internal int ConvertRiderRating(int permanentTableRating)
        {
            ...
            switch (permanentTableRating)
            {
                ...
                case 8:
                    return 3;
                default:
                    throw new PolicyConverterException($"ConvertRiderRating failed to process invalid permanentTableRating: {permanentTableRating} while executing the Common Policy Converter.");
            }
        ...
        internal int CalculateAttainedAge(DateTime dateOfBirth, DateTime issueDate, int issueAge)
        {
            if (log.IsInfoEnabled) log.Info($"CalculateAttainedAge called with dateOfBirth: {dateOfBirth.ToShortDateString()} issueAge: {issueAge.ToString()}.");

            int duration = 0;

            if (DateTime.Today.Month >= issueDate.Month)
            {
                duration = DateTime.Today.Year - issueDate.Year + 1;
            }
            else
            {
                duration = DateTime.Today.Year - issueDate.Year;
            }

            return issueAge + duration - 1;
        }
    }
}