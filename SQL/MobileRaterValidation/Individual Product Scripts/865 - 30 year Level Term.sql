...
as (
select *
from (select 865 as HeaderCode) vHeader
cross join (
    select StateGroupId
    from ( values (1003, 'Non-NY')) as StatesTable(StateGroupId, [Description])) vState
cross join (
    select MinInsuredAge, MaxInsuredAge
    from ( values (0, 15), (16, 17), (18, 50), (51, 55)) as InsuredAgeTable(MinInsuredAge, MaxInsuredAge)) vAge
cross join (
...
where ((StateGroupId = 1003 and MinInsuredAge >= 0 and MaxInsuredAge <= 55 and IsTobaccoUser = 0)
    or (StateGroupId = 1003 and MinInsuredAge >= 0 and MaxInsuredAge <= 50 and IsTobaccoUser = 1))
...
and (MinRatingClass >= -2 and MaxRatingClass <= 8)
and (ADRating = 0
