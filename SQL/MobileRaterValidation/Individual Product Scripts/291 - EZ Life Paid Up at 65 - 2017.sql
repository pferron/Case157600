...
as (
select *
from (select 291 as HeaderCode) vHeader
cross join (
    select StateGroupId
    from ( values (1019, 'Non-MT/MD/NY'), (105, 'MD')) as StatesTable(StateGroupId, [Description])) vState
cross join (
    select MinInsuredAge, MaxInsuredAge
    from ( values (0, 17), (18, 49), (50, 55)) as InsuredAgeTable(MinInsuredAge, MaxInsuredAge)) vAge
cross join (
...
Where 
    ((StateGroupId = 105 and MinInsuredAge >= 18 and MaxInsuredAge <= 55) 
    or (StateGroupId = 1019 and MinInsuredAge >= 0 and MaxInsuredAge <= 55))
 and  (
       (MinFaceAmount >= 5000 and MaxFaceAmount <= 5000 and MinInsuredAge >= 50 and MaxInsuredAge <= 55)
    or (MinFaceAmount >= 10000 and MaxFaceAmount <= 20000 and MinInsuredAge >= 0 and MaxInsuredAge <= 55)
    )
and (BillingMethod != 'D' or BillingFrequency != 'M')
and (ADRating = 0 
    or (ADRating = 1 and MinRatingClass >= 0 and MaxRatingClass <= 0) 
