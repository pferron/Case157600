...
SET @WPRating = 0 -- x2
SET @WMDRating = 0 -- No
SET @BaseRating = 2 -- Standard
;

WITH TempTable
as (
select *
from (select 855 as HeaderCode) vHeader
cross join (
    select StateGroupId
    from ( values (1003, 'Non-NY')) as StatesTable(StateGroupId, [Description])) vState
cross join (
    select MinInsuredAge, MaxInsuredAge
    from ( values (18, 55), (56, 60), (61, 65), (66, 70), (71, 75)) as InsuredAgeTable(MinInsuredAge, MaxInsuredAge)) vAge
cross join (
    select IsTobaccoUser
    from ( values (0), (1)) as IsTobaccoUserTable(IsTobaccoUser)) vTobacco
cross join (
    select MinRatingClass, MaxRatingClass
    from ( values (-2, -2, 'Super'), (-1, -1, 'Preferred'), (0, 0, 'Standard'), (1, 1, 'Table 1'),
        (2, 2, 'Table 2'), (3, 3, 'Table 3'), (4, 4, 'Table 4'), (5, 5, 'Table 5'),
...
where ((StateGroupId = 1003 and MinInsuredAge >= 18 and MaxInsuredAge <= 75))
...
and ((MinInsuredAge >= 18 and MaxInsuredAge <= 60 and MinRatingClass >= -2 and MaxRatingClass <= 8)
    or (MinInsuredAge >= 61 and MaxInsuredAge <= 65 and Gender = 'M' and MinRatingClass >= -2 and MaxRatingClass <= 6 and IsTobaccoUser = 0) 
    or (MinInsuredAge >= 61 and MaxInsuredAge <= 65 and Gender = 'M' and MinRatingClass >= -2 and MaxRatingClass <= 4 and IsTobaccoUser = 1) 
    or (MinInsuredAge >= 61 and MaxInsuredAge <= 65 and Gender = 'F' and MinRatingClass >= -2 and MaxRatingClass <= 8) 
    or (MinInsuredAge >= 66 and MaxInsuredAge <= 70 and Gender = 'M' and MinRatingClass >= -2 and MaxRatingClass <= 2) 
    or (MinInsuredAge >= 66 and MaxInsuredAge <= 70 and Gender = 'F' and MinRatingClass >= -2 and MaxRatingClass <= 6 and IsTobaccoUser = 0) 
    or (MinInsuredAge >= 66 and MaxInsuredAge <= 70 and Gender = 'F' and MinRatingClass >= -2 and MaxRatingClass <= 4 and IsTobaccoUser = 1) 
    or (MinInsuredAge >= 71 and MaxInsuredAge <= 75 and Gender = 'M' and MinRatingClass >= -2 and MaxRatingClass <= 0) 
    or (MinInsuredAge >= 71 and MaxInsuredAge <= 75 and Gender = 'F' and MinRatingClass >= -2 and MaxRatingClass <= 2))
and (ADRating = 0
    or ((MinInsuredAge >= 18 and MaxInsuredAge <= 65)
        and (MinFaceAmount >= 25000 and MaxFaceAmount <= 99999)
...
and (WPRating = 0
    or ((MinInsuredAge >= 18 and MaxInsuredAge <= 55)
