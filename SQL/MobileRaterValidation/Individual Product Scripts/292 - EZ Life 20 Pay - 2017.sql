...
as (
select *
from (select 292 as HeaderCode) vHeader
cross join (
    select StateGroupId
    from ( values (1019, 'Non-MT/MD/NY'), (105, 'MD')) as StatesTable(StateGroupId, [Description])) vState
cross join (
    select MinInsuredAge, MaxInsuredAge
    from ( values (0, 17), (18, 49), (50, 65), (66, 70)) as InsuredAgeTable(MinInsuredAge, MaxInsuredAge)) vAge
cross join (
...
where 
    ((StateGroupId = 105 and MinInsuredAge >= 18 and MaxInsuredAge <= 70) 
    or (StateGroupId = 1019 and MinInsuredAge >= 0 and MaxInsuredAge <= 70))
and	(  (MinFaceAmount >= 5000 and MaxFaceAmount <= 5000 and MinInsuredAge >= 50 and MaxInsuredAge <= 70)
    or (MinFaceAmount >= 10000 and MaxFaceAmount <= 20000 and MinInsuredAge >= 0 and MaxInsuredAge <= 70)
    )
