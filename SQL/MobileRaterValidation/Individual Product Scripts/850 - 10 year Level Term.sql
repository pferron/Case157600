...
as (
select *
from (select 850 as HeaderCode) vHeader
cross join (
    select StateGroupId
    from ( values (1003, 'Non-NY')) as StatesTable(StateGroupId, [Description])) vState
cross join (
    select MinInsuredAge, MaxInsuredAge
    from ( values (18, 55), (56, 60), (61, 65), (66, 70), (71, 80)) as InsuredAgeTable(MinInsuredAge, MaxInsuredAge)) vAge
cross join (
...
where ((StateGroupId = 1003 and MinInsuredAge >= 18 and MaxInsuredAge <= 80))
...
and ((MinInsuredAge >= 18 and MaxInsuredAge <= 60 and MinRatingClass >= -2 and MaxRatingClass <= 8) 
    ...
    or (MinInsuredAge >= 71 and MaxInsuredAge <= 80 and Gender = 'M' and MinRatingClass >= -2 and MaxRatingClass <= 0) 
    or (MinInsuredAge >= 71 and MaxInsuredAge <= 80 and Gender = 'F' and MinRatingClass >= -2 and MaxRatingClass <= 2))
and (ADRating = 0
