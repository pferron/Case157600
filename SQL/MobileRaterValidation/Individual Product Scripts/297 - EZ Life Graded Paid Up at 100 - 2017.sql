...
SET @HeaderCode = 297
SET @StateGroupId = 1017  --'Non-MT/NY/OR'
SET @InsuredAge = 65
SET @IsTobaccoUser = 0
...
cross join (
    select StateGroupId
    from ( values (1017, 'Non-MT/NY/OR'), (104, 'OR')) as StatesTable(StateGroupId, [Description])) vState
cross join (
    select MinInsuredAge, MaxInsuredAge
    from ( values (50, 74), (75, 80)) as InsuredAgeTable(MinInsuredAge, MaxInsuredAge)) vAge
cross join (
...
where ((StateGroupId = 1017 and MinInsuredAge >= 50 and MaxInsuredAge <= 80)
    or (StateGroupId = 104 and MinInsuredAge >= 50 and MaxInsuredAge <= 74)
    )
