...
as (
select *
from (select 119 as HeaderCode) vHeader
cross join (
    select StateGroupId
    from ( values (1003, 'Non-NY')) as StatesTable(StateGroupId, [Description])) vState
cross join (
    select MinInsuredAge, MaxInsuredAge
    from ( values (18, 55), (56, 64), (65, 65), (66, 69), (70, 74), (75, 79), (80, 80), (81, 82), (83, 85)) as InsuredAgeTable(MinInsuredAge, MaxInsuredAge)) vAge
cross join (
...
and ((Gender = 'F' and MinRatingClass >= -2 and MaxRatingClass <= 
            case 
                when MinInsuredAge >= 18 and MaxInsuredAge <= 69  then 16
    ...
    or (Gender = 'M' and MinRatingClass >= -2 and MaxRatingClass <= 
            case 
                when MinInsuredAge >= 18 and MaxInsuredAge <= 64  then 16
