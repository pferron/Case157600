...
as (
select *
from (select 870 as HeaderCode) vHeader
cross join (
    select StateGroupId
    from ( values (1003, 'Non-NY')) as StatesTable(StateGroupId, [Description])) vState
cross join (
    select MinInsuredAge, MaxInsuredAge
    from ( values (16, 17), (18, 55), (56, 60), (61, 65)) as InsuredAgeTable(MinInsuredAge, MaxInsuredAge)) vAge
cross join (
...
where ((StateGroupId = 1003 and MinInsuredAge >= 16 and MaxInsuredAge <= 65))
...
and ((MinInsuredAge >= 16 and MaxInsuredAge <= 60 and MinRatingClass >= -2 and MaxRatingClass <= 8) 
    or (MinInsuredAge >= 61 and MaxInsuredAge <= 65 and MinRatingClass >= -2 and MaxRatingClass <= 8 and IsTobaccoUser = 0) 
    or (MinInsuredAge >= 61 and MaxInsuredAge <= 65 and MinRatingClass >= -2 and MaxRatingClass <= 6 and IsTobaccoUser = 1))
and (ADRating = 0
