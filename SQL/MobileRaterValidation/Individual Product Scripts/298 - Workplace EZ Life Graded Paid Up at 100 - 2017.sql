...
SET @HeaderCode = 298
SET @StateGroupId = 1018 -- 'Non-NY/OR'
SET @InsuredAge = 65
SET @IsTobaccoUser = 0
...
cross join (
    select StateGroupId
    from ( values (1018, 'Non-NY/OR'), (104, 'OR')) as StatesTable(StateGroupId, [Description])) vState
cross join (
...
where (
       (StateGroupId = 1018 and MinInsuredAge >= 50 and MaxInsuredAge <= 80)
