...
/****** Object:  View [dbo].[vCA_NY_WL2017_503_282]    Script Date: 7/12/2018 2:44:33 PM ******/
SET ANSI_NULLS ON
GO
...
Create View [dbo].[vCA_NY_WL2017_503_282]
as (
select *
...
cross join (
    ...
    from ( values (0, 15), (16, 17), (18, 37), (38, 55), (56, 64), (65, 65), (66, 69), (70, 70)) as InsuredAgeTable(MinInsuredAge, MaxInsuredAge)) vAge
cross join (
...
cross join (
    ...
    from ( values (-2, -2, 'Super'), (-1, -1, 'Preferred'), (0, 1, 'Standard to Table 1'), (2, 3, 'Table 2 & 3'), (4, 4, 'Table 4'), 
        (5, 5, 'Table 5'), (6, 6, 'Table 6'), (7, 7, 'Table 7'), (8, 10, 'Table 8 to 10'), (11, 12, 'Table 11 & 12'), 
...
and ((Gender = 'F' and MinRatingClass >= -2 and MaxRatingClass <= 
    ...
    or (Gender = 'M' and MinRatingClass >= -2 and MaxRatingClass <= 
            case 
                ...
                when MinInsuredAge >= 70 and MaxInsuredAge <= 70 then 10
            end))
and (ADRating = 0
