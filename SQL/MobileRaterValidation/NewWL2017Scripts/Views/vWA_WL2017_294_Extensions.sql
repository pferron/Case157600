Create View WA_WL2017_294_Extensions
as
Select *
from (select 294 as HeaderCode) vHeader
cross join (
    select StateGroupId
    from ( values (504, 'WA-WL2017 Extensions')) as StatesTable(StateGroupId, [Description])) vState
cross join (
    select MinInsuredAge, MaxInsuredAge
    from ( values (16, 49), (50, 65), (66, 80), (81, 85)) as InsuredAgeTable(MinInsuredAge, MaxInsuredAge)) vAge
cross join (
...
where (--TBD
       (MinFaceAmount >= 5000 and MaxFaceAmount <= 5000 and MinInsuredAge >= 50 and MaxInsuredAge <= 85 and MinRatingClass >= 0 and MaxRatingClass <= 0)
    or (MinFaceAmount >= 5000 and MaxFaceAmount <= 5000 and MinInsuredAge >= 50 and MaxInsuredAge <= 80 and MinRatingClass >= 1 and MaxRatingClass <= 1)
    or (MinFaceAmount >= 10000 and MaxFaceAmount <= 20000 and MinInsuredAge >= 16 and MaxInsuredAge <= 85 and MinRatingClass >= 0 and MaxRatingClass <= 0)
    or (MinFaceAmount >= 10000 and MaxFaceAmount <= 20000 and MinInsuredAge >= 16 and MaxInsuredAge <= 80 and MinRatingClass >= 1 and MaxRatingClass <= 1)
    )
and (--TBD
	ADRating = 0 
